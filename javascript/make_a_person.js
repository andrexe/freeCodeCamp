...
const Person = function (firstAndLast) {
    ...
    // Declare the name variable LOCALLY so it doesn't count as a key and pass it the original argument value
    let fullName = firstAndLast;
    this.getFullName = function () {
        return fullName;
    };
    // Split the string by space ' ' and either shift or pop to get first / last name
    ...
    // Use the same split, shift and pop methods to reconstruct the full name using the new argument
    this.setFirstName = function (first) {
        console.log(first);
        fullName = first + ' ' + fullName.split(' ').pop();
        console.log(fullName.split(' ').shift())
    };
